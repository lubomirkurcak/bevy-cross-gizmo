...
pub trait BevyCrossGizmo {
    fn cross(&mut self, position: Vec3, rotation: Quat, half_size: f32, color: impl Into<Color>);
    fn cross_2d(&mut self, position: Vec2, angle: f32, half_size: f32, color: impl Into<Color>);
}
